# Taken from Alexander Kireev's kernel:
# https://www.kaggle.com/alexanderkireev/deep-learning-support-9663/code
# Resources:
...
if not os.path.isfile('train_df.csv'):
    print('load train....')
    # we save only day 9
    train_df = pd.read_csv(path + "train.csv.zip", compression='infer',
                           dtype=dtypes,
#                            skiprows=range(1, 131886954),
                           skiprows=range(1,159000000),
                           usecols=['ip', 'app', 'device', 'os', 'channel',
                                    'click_time', 'is_attributed'])
    print('load test....')
    test_df = pd.read_csv(path + "test.csv.zip", compression='infer', dtype=dtypes,
                          usecols=['ip', 'app', 'device', 'os', 'channel',
                                   'click_time', 'click_id'])
    ...
    print("Loaded formatted train dataframe")
...
train_df.drop(['click_id', 'click_time', 'ip',
               'is_attributed'], 1, inplace=True)
...
max_c1 = np.max([train_df['ip_app_count'].max(),
                 test_df['ip_app_count'].max()]) + 1
max_c2 = np.max([train_df['ip_app_os_count'].max(),
                 test_df['ip_app_os_count'].max()]) + 1
...
model.fit(train_df, y_train, batch_size=batch_size,
          epochs=2, shuffle=True, verbose=2)
...
test_df.drop(['click_id', 'click_time', 'ip',
              'is_attributed'], 1, inplace=True)
